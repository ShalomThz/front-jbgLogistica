...
export const messageFallbacks: Array<{ pattern: RegExp; code: string }> = [
  { pattern: /email.*already.*taken|already.*taken.*email|email already in use/i, code: "EMAIL_ALREADY_TAKEN" },
  ...
  { pattern: /phone.*already.*use|already.*use.*phone/i, code: "CUSTOMER_PHONE_ALREADY_EXISTS" },
  { pattern: /box.*already exists|already exists.*box/i, code: "BOX_NAME_ALREADY_EXISTS" },
