...
export const authRepository = {
  login: async (
    ...
    if (!result.success) {
      console.error("[authRepository] login: respuesta inválida", result.error.issues);
      throw result.error;
    }
  ...
  getCurrentUser: async (): Promise<UserListViewPrimitives> => {
    ...
    if (!result.success) {
      console.error("[authRepository] getCurrentUser: respuesta inválida", result.error.issues);
      throw result.error;
    }
